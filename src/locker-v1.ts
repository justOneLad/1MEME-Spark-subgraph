...
import {
  ...
  CTORejected,
  SparkLocker,
} from "../generated/SparkLockerV1/SparkLocker";
import { LockerPositionV1, LockerFeeClaimV1, CTOApplication } from "../generated/schema";
import { ZERO_BI, ZERO_ADDRESS, CTO_SOURCE_LOCKER_V1, eventId, getOrCreateStats } from "./helpers";

export function handlePositionRegistered(event: PositionRegistered): void {
  let position = new LockerPositionV1(event.params.token);
...
export function handleFeesClaimed(event: FeesClaimed): void {
  let position = LockerPositionV1.load(event.params.token);
  ...
  let claim = new LockerFeeClaimV1(eventId(event));
...
export function handleCTOApplied(event: CTOApplied): void {
  ...
  let id = CTO_SOURCE_LOCKER_V1 + "-" + eventId(event).toHexString();
  let application = new CTOApplication(id);
  application.source = CTO_SOURCE_LOCKER_V1;
  application.tokenV1 = event.params.token;
  application.positionV1 = event.params.token;
